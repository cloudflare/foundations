...
type IgnoredKeyCallback = Box<dyn FnMut(IgnoredPath)>;
...
pub struct SettingsDeserializationOptionsBuilder {
    ignored_key_callback: Option<IgnoredKeyCallback>,
}

impl Default for SettingsDeserializationOptionsBuilder {
    fn default() -> Self {
        Self::new()
    }
}
...
impl SettingsDeserializationOptionsBuilder {
    ...
    ///
    /// ```
    pub fn with_ignored_key_callback(mut self, callback: IgnoredKeyCallback) -> Self {
...
pub struct SettingsDeserializationOptions {
    ignored_key_callback: IgnoredKeyCallback,
}
