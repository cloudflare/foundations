...
fn bench_uncontended_low_rate(c: &mut Criterion) {
    ...
    group.bench_function("1_per_min_1_burst", |b| {
        ...
        b.iter(|| black_box(limiter.ratelimited()))
    });
...
fn bench_contended_low_rate(c: &mut Criterion) {
    ...
    for num_threads in [2, 4, 8, 16] {
        group.bench_with_input(
            ...
            &num_threads,
            |b, &num_threads| {
                let limiter = Arc::new(RateLimiter::new(1.0 / 60.0, 1));
                        ...
                        .map(|_| {
                            ...
                            std::thread::spawn(move || {
                                ...
                                for _ in 0..iters_per_thread {
                                    let _ = black_box(limiter.ratelimited());
                                }
                                start.elapsed()
                            })
                        })
...
fn bench_contended_high_rate(c: &mut Criterion) {
    ...
    for num_threads in [2, 4, 8, 16] {
        group.bench_with_input(
            ...
            &num_threads,
            |b, &num_threads| {
                let limiter = Arc::new(RateLimiter::new(1_000.0, 10));
