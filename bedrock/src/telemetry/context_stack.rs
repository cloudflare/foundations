...
pub(crate) struct ContextStack<T>(ThreadLocal<RefCell<Vec<T>>>)
where
    T: Send + Clone + 'static;

impl<T> Default for ContextStack<T>
where
    T: Send + Clone + 'static,
{
    fn default() -> Self {
...
pub(crate) struct CurrentContextHandle<T>
where
    T: Send + Clone + 'static,
{
    ctx_stack: &'static ContextStack<T>,
    await_point_guard: AwaitPointGuard,
}

impl<T> CurrentContextHandle<T>
where
    T: Send + Clone + 'static,
{
    pub(crate) fn new(ctx_stack: &'static ContextStack<T>, item: T) -> Self {
        ctx_stack.0.get_or_default().borrow_mut().push(item);
...
impl<T> Drop for CurrentContextHandle<T>
where
    T: Send + Clone + 'static,
{
    fn drop(&mut self) {
        self.ctx_stack.0.get_or_default().borrow_mut().pop();
    }
}
